...
if __name__ == "__main__":
    ...
    all_ff = []
    for p in profiles[:5]:
        frequencies, formfactors, det_lim = get_crisp_signal(
            s * 1e-6, p, n_shots=10, which_set="both"
        )
    ...
    for longer in elongate_factors:
        this_s = s * longer
        frequencies, formfactor, det_lim = get_crisp_signal(
            this_s * 1e-6, profiles[3] / longer, n_shots=50, which_set="both"
        )
        t, cur, rms = master_recon(
            frequencies,
            formfactor,
            ...
            channels_to_remove=[],
            wanted_time_res=2e-15,
            wanted_time_frame=3e-12,
            high_inter_last=1,
            model_last_index=-1,
            smooth_window=5,
            extra_low_ff_value=0.5,
            show_plots=True,
        )
    ...
    for longer in elongate_factors:
        ...
        plt.plot(
            ...
            label=str(longer_rms[i] * 1e15) + " fs",
        )
        i = i + 1
    plt.legend()

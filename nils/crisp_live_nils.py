...
def get_real_crisp_data(shots=20, which_set="both", nbunch=0):
    ...
    for shot in np.arange(shots):
        t0 = time.time()
        all_data[shot] = pydoocs.read(crisp_adress + "FORMFACTOR.ARRAY")["data"][
            :, nbunch * 2
        ]
        t1 = time.time()
        dt = t1 - t0
    ...
    det_lim = pydoocs.read(crisp_adress + "FORMFACTOR_MEAN_DETECTLIMIT.XY")["data"][
        :, 1
    ]
    ...
    det_lim = np.sqrt(
        det_lim * np.sqrt(10 / shots)
    )  # (1 sigma in electronic noise?! und auf richtige schussanzahl anpassen

...
def get_stage_position():
    ...
    for n in np.arange(np.size(stage_motors)):
        outcome[n] = pydoocs.read(stage_motors_adresse + stage_motors[n] + "/FPOS")[
            "data"
        ]
...
def push_stage_position(input_string):
    ...
    for n in np.arange(np.size(stage_motors)):
        ...
        while (
            int(
                format(
                    pydoocs.read(stage_motors_adresse + stage_motors[n] + "/MSTATUS")[
                        "data"
                    ],
                    "06b",
                )[-3]
...
def select_bunch(bunch_number):
    ...
    """
    pydoocs.write(
        "XFEL.SDIAG/THZ_SPECTROMETER.FORMFACTOR/CRD.1934.TL/NTH_BUNCH", bunch_number
    )
    time.sleep(1)
    selected_bunch = pydoocs.read(
        "XFEL.SDIAG/THZ_SPECTROMETER.FORMFACTOR/CRD.1934.TL/NTH_BUNCH"
    )["data"]
...
def get_reconstruction():
    ...
    """
    data = pydoocs.read(
        "XFEL.SDIAG/THZ_SPECTROMETER.RECONSTRUCTION/CRD.1934.TL/OUTPUT_TIMES"
    )
    current = pydoocs.read(
        "XFEL.SDIAG/THZ_SPECTROMETER.RECONSTRUCTION/CRD.1934.TL/CURRENT_PROFILE"
    )["data"]
    time = data["data"]
    return [time, current]

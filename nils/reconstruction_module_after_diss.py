...
def remove_loners(
    ...
    """
    mind_error = 0.1 * new_sorted_formfactors
    new_sorted_noise_plus[new_sorted_noise_plus < mind_error] = mind_error[
        new_sorted_noise_plus < mind_error
    ]
    new_sorted_noise_minus[new_sorted_noise_minus < mind_error] = mind_error[
        new_sorted_noise_minus < mind_error
    ]

    return (
        sorted_frequencies[my_maske],
        new_sorted_formfactors[my_maske],
        new_sorted_noise_plus[my_maske],
...
def extrapolation_low(
    ...
    sorted_frequencies,
    sorted_formfactors,
    sorted_formfactor_noise_plus,
    sorted_formfactor_noise_minus,
):
    """
    Extrapolation to low Frequencies. Gaussian to last 30 significant points
    Question: Allow Formfactors bigger than 1? -> No
    ...
    new_formfactors = np.append(add_ff, sorted_formfactors)
    new_noise_plus = np.append(np.zeros(np.size(add_ff)), sorted_formfactor_noise_plus)
    new_noise_minus = np.append(
        np.zeros(np.size(add_ff)), sorted_formfactor_noise_minus
    )
    return new_freqs, new_formfactors, new_noise_plus, new_noise_minus
...
def smooth_formfactors(
    frequencies,
    formfactors,
    formfactors_noise_plus,
    formfactors_noise_minus,
    window_size=9,
):
    """
    ...
    for channel in np.arange(np.size(frequencies)):
        if (
            new_formfactors[channel] + formfactors_noise_plus[channel]
            < formfactors[channel]
        ):
            new_ff_noise_plus[channel] = formfactors[channel] - new_formfactors[channel]
        if (
            new_formfactors[channel] - formfactors_noise_minus[channel]
            > formfactors[channel]
        ):
            new_ff_noise_minus[channel] = (
                -formfactors[channel] + new_formfactors[channel]
            )
...
def make_linear_spacing(
    ...
    sorted_frequencies,
    sorted_formfactors,
    sorted_formfactors_noise_plus,
    sorted_formfactors_noise_minus,
):
    # Interpolation
    ...
    g = interpolate.interp1d(
        sorted_frequencies, sorted_formfactors_noise_plus, bounds_error=True
    )
    new_noise_plus = g(new_frequencies)
    g = interpolate.interp1d(
        sorted_frequencies, sorted_formfactors_noise_minus, bounds_error=True
    )
...
def gerchberg_saxton(
    ...
    formfactors_noise_minus,
    start_phase,
    phase_condition=np.pi / 0.01,
    give_all_iterations=False,
):
    """
...
def model_start(
    ...
    model_ff,
    mess_formfactors,
    mess_formfactors_noise_plus,
    mess_formfactors_noise_minus,
    give_all_iterations=False,
):
    """
    ...
    if give_all_iterations == True:
        time_domain, all_iterations = gerchberg_saxton(
            ...
            mess_formfactors_noise_minus,
            start_phase,
            give_all_iterations=give_all_iterations,
        )
...
def master_recon(
    ...
    wanted_time_res=None,
    wanted_time_frame=None,
    high_inter_last=1,
    model_last_index=-1,
    smooth_window=9,
    ...
    (
        sign_frequencies,
        sign_formfactors,
        sign_formfactors_noise_plus,
        sign_formfactors_noise_minus,
        maske,
    ) = remove_loners(sort_freq, sorted_stuff[0], sorted_stuff[1], sorted_stuff[2])
    # mach das frequenz finden dynamisch:
    # by gauss fit durch den ersten gleiten mittelwert <0.7
    if wanted_time_res == None:
        wanted_time_frame, wanted_time_res = get_timing_grid(
            sign_frequencies, sign_formfactors
        )
    ...
    (
        extra_low_freqs,
        extra_low_ff,
        extra_low_ff_noise_plus,
        extra_lwo_ff_noise_minus,
    ) = extrapolation_low(
        new_freqs,
        sign_frequencies,
        sign_formfactors,
    ...
    (
        extra_high_freqs,
        extra_high_ff,
        extra_high_ff_noise_plus,
        extra_high_ff_noise_minus,
    ) = extrapolation_high(
        new_freqs,
        extra_low_freqs,
        extra_low_ff,
    ...
    final_ff, final_ff_noise_plus, final_ff_noise_minus = make_linear_spacing(
        new_freqs,
        extra_high_freqs,
        smooth_ff,
        extra_high_ff_noise_plus,
    ...
    min_dev = max_vary
    final_ff_noise_plus[final_ff_noise_plus < min_dev * final_ff] = (
        final_ff[final_ff_noise_plus < min_dev * final_ff] * min_dev
    )
    final_ff_noise_minus[final_ff_noise_minus < min_dev * final_ff] = (
        final_ff[final_ff_noise_minus < min_dev * final_ff] * min_dev
    )
    ...
    ):  # wenn kein string, dann direkt mit einer phase starten: x sind die freqs, y die phase
        phasefreqs = method[0]
    ...
    # Reconstruction
    recon_time, recon_prof = model_start(
        new_freqs, start_ff, final_ff, final_ff_noise_plus, final_ff_noise_minus
    )
    ...
    if show_plots == True:
        ...
        axes[0].errorbar(
            new_freqs * 1e-12,
            final_ff,
            yerr=[final_ff_noise_minus, final_ff_noise_plus],
            fmt="-",

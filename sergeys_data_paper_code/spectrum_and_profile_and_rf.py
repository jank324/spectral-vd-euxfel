...
for i, chirp in enumerate(rf_params["chirp"]):
    print(f"L1: V = {rf_params['XFEL.RF/LLRF.SUMVOLTAGE_CTRL/L1/SUMVOLTAGE.AMPLITUDE.SP.1'][i]}, chirp = {rf_params['chirp'][i]}")

# show only one others are the same
for i, chirp in enumerate(rf_params["chirp"]):
    print(f"Injector: V = {rf_params['XFEL.RF/LLRF.SUMVOLTAGE_CTRL/I1/SUMVOLTAGE.AMPLITUDE.SP.1'][i]}, "
          f"chirp = {rf_params['XFEL.RF/LLRF.SUMVOLTAGE_CTRL/I1/SUMVOLTAGE.CHIRP.SP.1'][i]}, "
          f"curv = {rf_params['XFEL.RF/LLRF.SUMVOLTAGE_CTRL/I1/SUMVOLTAGE.CURVATURE.SP.1'][i]}, "
          f"skew = {rf_params['XFEL.RF/LLRF.SUMVOLTAGE_CTRL/I1/SUMVOLTAGE.THIRDDERIVATIVE.SP.1'][i]}, ")
    print("Injector: Others are the same. break ... \n")
    break

for i, chirp in enumerate(rf_params["chirp"]):
    print(f"L1: V = {rf_params['XFEL.RF/LLRF.SUMVOLTAGE_CTRL/L2/SUMVOLTAGE.AMPLITUDE.SP.1'][i]}, chirp = {rf_params['XFEL.RF/LLRF.SUMVOLTAGE_CTRL/L2/SUMVOLTAGE.CHIRP.SP.1'][i]}")
    print("Injector: Others are the same. break ... \n")
    break
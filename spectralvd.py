...
class Fixed:
    ...
    def fit(self, formfactors, currents, epochs=1000, verbose=1):
        ...
        self.history = self.model.fit(
            X_scaled, y_scaled, epochs=epochs, batch_size=64, verbose=verbose
        )
    ...
    def _preprocess_currents(self, currents):
        shifted = [
            self._shift_onto_center_of_mass(s, current) for s, current in currents
        ]

        new_s = np.linspace(-self.width / 2, self.width / 2, self.n_samples)
        interpolated = [
            (new_s, np.interp(new_s, s, current, left=0, right=0))
            for s, current in shifted
        ]
...
class Adaptive:
    ...
    def _preprocess_currents(self, currents):
        ...
        resampled = [
            resample(s, current, left, right, self.n_samples)
            for (s, current), (left, right) in zip(currents, edges)
        ]
        centered = [
            center_on_zero(s, current, left, right)
            for (s, current), (left, right) in zip(resampled, edges)
        ]
...
class AdaptiveANNRF(AdaptiveANN):
    def fit(self, rf, currents, epochs=1000, verbose=1):
        ...
        self.history1 = self.model1.fit(
            X_scaled, y1_scaled, epochs=epochs, batch_size=64, verbose=verbose
        )
        self.history2 = self.model2.fit(
            X_scaled, y2_scaled, epochs=epochs, batch_size=64, verbose=verbose
        )

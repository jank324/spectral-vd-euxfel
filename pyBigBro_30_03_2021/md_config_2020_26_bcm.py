...
snapshot.add_orbit_section("B2D", tol=0.1, track=False)
snapshot.add_orbit_section("B2", tol=0.1, track=False)
snapshot.add_orbit_section("L2", tol=0.1, track=False)

snapshot.add_magnet_section("L2", tol=0.01, track=False)
snapshot.add_magnet_section("B2", tol=0.01, track=False)
snapshot.add_magnet_section("B2D", tol=0.01, track=False)
# add camera 
...
snapshot.add_image("XFEL.DIAG/CAMERA/OTRB.457.B2/IMAGE_EXT_ZMQ", folder="./tds_images")
# solenoid
...
START: Nils
"""
#THz Spectrometer: orbit,charge,stage position, screen position, 
snapshot.add_orbit_section("TL", tol=0.1, track=False)#orbit
snapshot.add_channel("XFEL.DIAG/CHARGE.ML/BPMI.1930.TL/CHARGE.ALL")#charge of first bunch
snapshot.add_channel("XFEL.SDIAG/SPS/MOTOR1.1934.TL/POS")#screen mover position
snapshot.add_channel("XFEL.SDIAG/THZ_SPECTROMETER.FORMFACTOR/CRD.1934.TL/INTENSITY.TD")#all intensities at first bunch
snapshot.add_channel("XFEL.SDIAG/THZ_SPECTROMETER.FORMFACTOR/CRD.1934.TL/FORMFACTOR.XY")#formfactor at first bunch
snapshot.add_channel("XFEL.SDIAG/THZ_SPECTROMETER.FORMFACTOR/CRD.1934.TL/FORMFACTOR_DETECTLIMIT.XY")#detection limit
snapshot.add_channel("XFEL.SDIAG/THZ_SPECTROMETER.FORMFACTOR/CRD.1934.TL/STAGE")#which grating set is used

#BCMs
snapshot.add_orbit_section("B1", tol=0.1, track=False)
snapshot.add_channel("XFEL.SDIAG/BCM/BCM.180.B1/MCT.SA1")
snapshot.add_channel("XFEL.SDIAG/BCM/BCM.180.B1/PYRO.SA1")
snapshot.add_channel("XFEL.SDIAG/BCM/BCM.205.B1/MCT.SA1")
snapshot.add_channel("XFEL.SDIAG/BCM/BCM.205.B1/PYRO.SA1")
snapshot.add_channel("XFEL.SDIAG/BCM/BCM.416.B2/MCT.SA1")
snapshot.add_channel("XFEL.SDIAG/BCM/BCM.416.B2/PYRO.SA1")
snapshot.add_channel("XFEL.FEEDBACK/FT1.LONGITUDINAL/MONITOR12/MEAN")
snapshot.add_channel( "XFEL.FEEDBACK/FT1.LONGITUDINAL/MONITOR13/MEAN")

#RF - L1
snapshot.add_channel("XFEL.RF/LLRF.CONTROLLER/VS.A2.L1/PHASE.SAMPLE")
snapshot.add_channel("XFEL.RF/LLRF.CONTROLLER/VS.A2.L1/AMPL.SAMPLE")
...
#RF-L2
for i in range(3,6):
	snapshot.add_channel("XFEL.RF/LLRF.CONTROLLER/VS.A" + str(i) + ".L2/PHASE.SAMPLE")
	snapshot.add_channel("XFEL.RF/LLRF.CONTROLLER/VS.A" + str(i) + ".L2/AMPL.SAMPLE")
	snapshot.add_channel("XFEL.RF/LLRF.CONTROLLER/CTRL.A" + str(i) + ".L2/SP.PHASE")
	snapshot.add_channel("XFEL.RF/LLRF.CONTROLLER/CTRL.A" + str(i) + ".L2/SP.AMPL")
#RF-L3
for i in range(6,25):
	snapshot.add_channel("XFEL.RF/LLRF.CONTROLLER/VS.A" + str(i) + ".L3/PHASE.SAMPLE")
	snapshot.add_channel("XFEL.RF/LLRF.CONTROLLER/VS.A" + str(i) + ".L3/AMPL.SAMPLE")
	snapshot.add_channel("XFEL.RF/LLRF.CONTROLLER/CTRL.A" + str(i) + ".L3/SP.PHASE")
	snapshot.add_channel("XFEL.RF/LLRF.CONTROLLER/CTRL.A" + str(i) + ".L3/SP.AMPL")
...
snapshot.add_channel("XFEL.DIAG/BEAM_ENERGY_MEASUREMENT/CL/ENERGY.ALL")
